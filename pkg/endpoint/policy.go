...
func (e *Endpoint) removeOldFilter(owner Owner, labelsMap *LabelsMap,
	...
	for _, sel := range filter.FromEndpoints {
		for _, id := range getSecurityIdentities(labelsMap, &sel) {
			srcID := id.Uint32()
			ruleCtx := policy.ParseL4Filter(*filter)
			if _, ok := fromEndpointsSrcIDs[id]; !ok {
				fromEndpointsSrcIDs[id] = policy.NewRuleContexts()
			}
			if err := e.PolicyMap.DeleteL4(srcID, port, proto); err != nil {
				...
				v, ok := fromEndpointsSrcIDs[id][ruleCtx]
				if ok && !v {
					fromEndpointsSrcIDs[id][ruleCtx] = false
				}
			} else {
				fromEndpointsSrcIDs[id][ruleCtx] = true
			}
		}
	}
...
// policy.
// Applies for L4 dependent L3 not for L4-only.
	...
	for identity, ruleContexts := range newSecIDs {
		for ruleContext, v := range ruleContexts {
			if _, ok := secIDs[identity]; !ok {
				secIDs[identity] = policy.NewRuleContexts()
			}
			e, ok := secIDs[identity][ruleContext]
			secIDs[identity][ruleContext] = (v && !ok) || (v && e)
		}
	}
}
...
// and a map matching which rules were successfully added;
...
func (e *Endpoint) regenerateConsumable(owner Owner, labelsMap *LabelsMap,
	repo *policy.Repository, c *policy.Consumable) (bool, policy.SecurityIDContexts, policy.SecurityIDContexts) {
	...
	rulesAdd := policy.SecurityIDContexts{}
	rulesRm := policy.SecurityIDContexts{}
	...
	if e.L4Policy != c.L4Policy || e.LabelsMap != labelsMap {
		...
		// There's no way to know which L4 policies are L4-only
		// so we need to know which ones are L4-only
		if c != nil && c.L4Policy != nil && c.L4Policy.Ingress != nil {
			for _, l4Filter := range c.L4Policy.Ingress {
				found := false
				for _, ruleContexts := range rulesAdd {
					for ruleContext := range ruleContexts {
						if ruleContext.Proto == uint8(l4Filter.U8Proto) &&
							ruleContext.Port == uint16(l4Filter.Port) {
							found = true
							break
						}
					}
				...
				if !found {
					if _, ok := rulesAdd[policy.InvalidIdentity]; !ok {
						rulesAdd[policy.InvalidIdentity] = policy.NewRuleContexts()
					}
					ruleCtx := policy.ParseL4Filter(l4Filter)
					rulesAdd[policy.InvalidIdentity][ruleCtx] = true
				}
			}
		}
		...
		for secID, ruleCtxsRm := range l4Rm {
			if ruleCtxsAdd, ok := rulesAdd[secID]; ok {
				for ruleCtxAdd, add := range ruleCtxsAdd {
					if rm, ok := ruleCtxsRm[ruleCtxAdd]; rm && ok && !(add) {
						if _, ok := rulesRm[secID]; !ok {
							rulesRm[secID] = policy.NewRuleContexts()
						}
						rulesRm[secID][ruleCtxAdd] = true
					}
				}
			} else {
				...
				for k, v := range ruleCtxsRm {
					removed, ok := rulesRm[secID][k]
					rulesRm[secID][k] = v || (removed && ok)
				}
			}
		}
	} else {
		// If there were no modifications to thee L3L4 policy lets copy the
		// existing L3L4 policy.
		if c.L3L4Policy != nil {
			rulesAdd = c.L3L4Policy.DeepCopy()
		}
	}
	...
	for _, val := range c.Consumers {
		if val.DeletionMark {
			...
			if _, ok := rulesRm[val.ID]; !ok {
				rulesRm[val.ID] = policy.NewRuleContexts()
			}
		} else {
			...
			if _, ok := rulesAdd[val.ID]; !ok {
				rulesAdd[val.ID] = policy.NewRuleContexts()
			}
		}
	}

	//1. Check for for l3 rule, if none, reject
	//2. Check for l4 rule that is dependant on previous l3 rule, if present and no match, reject
	//3. Check for wildcard l4 rule, if present and no match, reject
	//4. Check list of l7 rules for given port and protocol regardless of any previous l3 rule, if present and no match, reject
	//5. reject
...
func (e *Endpoint) regeneratePolicy(owner Owner, opts models.ConfigurationMap) (bool, policy.SecurityIDContexts, policy.SecurityIDContexts, error) {
	...
	if !e.forcePolicyCompute && e.nextPolicyRevision >= revision &&
		...
		// REVIEW: Why does this means the policy was changed?!
