...
func doGC6(m *bpf.Map, filter *GCFilter) int {
	...
	for {
		...
		// which means, is the destination's IP.
		...
		case deleteEntry:
			...
			if err != nil {
				log.WithError(err).Error("Unable to delete CT entry %s", nextKey.String())
			} else {
				deleted++
			}
		}
...
func (f *GCFilter) doFiltering(dstIP net.IP, dstPort uint16, nextHdr, flags uint8, entry *CtEntry) (action int) {
	...
	if f.Type&GCFilterByIDsToKeep != 0 &&
		...
		// #### REVIEWER: what about TUPLE_F_RELATED?
		flags&TUPLE_F_IN != 0 {
		...
		if filterRuleCtx, ok := f.IDsToKeep[policy.NumericIdentity(entry.src_sec_id)]; !ok {
			...
			if filterRuleCtx.IsL3Only() {
				...
				if l4OnlyRules, ok := f.IDsToKeep[policy.InvalidIdentity]; ok {
					wasAdded, ok := l4OnlyRules[ruleCtx]
					if !ok || !wasAdded {
						action = deleteEntry
					}
				} else {
					for k, v := range l4OnlyRules {
						if k.IsRedirect &&
							k.Port == ruleCtx.Port &&
							k.Proto == ruleCtx.Proto &&
							v {
							action = modifyEntry
							entry.proxy_port = k.L7RedirectPort
						}
					}
				}
			} else {
				...
				// L3-L4 rules.
				if _, ok := filterRuleCtx[ruleCtx]; !ok {
					action = deleteEntry
					for k, v := range filterRuleCtx {
						if k.IsRedirect &&
							k.Port == ruleCtx.Port &&
							k.Proto == ruleCtx.Proto &&
							v {
